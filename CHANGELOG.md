# CHANGELOG

## v5.2-r2 (2026-09-14)
* \[UPDATE] New behat tests
* \[UPDATE] Changed old renderer to only work with Arial and horizontal orientation

## v5.2-r1 (2026-08-23)

* \[UPDATE] Langstring changes 
* Moodle 5.2 compatible version

## v5.1-r2 (2026-08-22)

* \[FIX] Fixed the moodle app view
* \[FEATURE] Added preview function in activity settings

## v5.1-r1 (2026-07-10)

* \[FIX] Fixed missing wordcloud activities in block timeline
* \[FIX] Fixed the less-than character "<" not accepted in submissions
* \[FIX] Fixed the navigation to the wordlist in classic theme
* \[FEATURE] Support further style options with renderer (font, text alignment)
* \[FEATURE] Update admin settings due to further style options
* \[FEATURE] Support sorting order in list view and on wordlist page
* \[FEATURE] Hide renderer area and wordlist if no word has been submitted yet
* \[FEATURE] Support further information in the activity overview
* Moodle 5.1 compatible version

## v5.0-r1 (2025-04-14)

* \[FEATURE] Add further behat tests
* \[UPDATE] Update of readme file
* Moodle 5.0 compatible version

## 4.5.0

* \[FIX] Bug fixes and improvements
* Moodle 4.5 compatible version
* Thanks to the contributors: dpalou, james-cnz

## 4.4.0

* Moodle 4.4 compatible version

## 4.3.1

* \[FEATURE] New activity completion settings
* \[FEATURE] New visibility settings
* \[FIX] Backup bugfixes

## 4.3.0

* \[FIX] Fixed some minor Bugs
* Moodle 4.3 compatible version

## 4.2.1

* \[FEATURE] Design changes
* Bug fixes

## 4.2.0

* \[FEATURE] Changed button layout
* \[FEATURE] Added listview
* Moodle 4.2 compatible version

## 4.1.0

* \[FEATURE] Dynamic wordcloud sizing
* \[FIX] Webservice parameter fix
* \[FEATURE] Added behat tests
* Moodle 4.1 compatible version

## 4.0.0

* \[FEATURE] Added activity setting to choose monocolor from six textcolors or a custom color value.
* \[FEATURE] Added group support
* Moodle 4.0 compatible version

## 3.11.2

* \[FIX] Fixed lang string for remove button

## 3.11.1

* \[FEATURE] Added moodle mobile app support
* \[FEATURE] Added edit entry page to customize entries
* \[FEATURE] Added a second color scheme (monochrome color scheme)
* \[FEATURE] Added timing display in course view
* \[UPDATE] Removed the background color of the wordcloud
* Last version with Moodle 3.10 support

## 3.11.0

* Moodle 3.11 compatible version

## 3.10.0

* Moodle 3.10 compatible version

